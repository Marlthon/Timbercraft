...
public class Location
{
    ...
    private Texture2D? loadTexture(string name)
    {
        try
        {
            if (ReadEmbeddedFileBytes(name) is { } textureData)
            {
                ...
                // CORREÇÃO para Unity 6: Usar reflexão para chamar LoadImage
                System.Type imageConversionType = typeof(Texture2D).Assembly.GetType("UnityEngine.ImageConversion");
                if (imageConversionType != null)
                {
                    MethodInfo loadImageMethod = imageConversionType.GetMethod("LoadImage", new[] { typeof(Texture2D), typeof(byte[]) });
                ...
                Debug.LogError($"Could not find ImageConversion.LoadImage method for texture '{name}'");
                return null;
            }
            return null;
        }
    ...
    static Location()
    {
        ...
        harmony.Patch(AccessTools.DeclaredMethod(typeof(Terminal), nameof(Terminal.InitTerminal)), postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(Location), nameof(AddConsoleCommands))));
    }

    private static void AddConsoleCommands()
    {
        new Terminal.ConsoleCommand("regen_locations", "Regenera todas as locations customizadas no mapa atual", (Terminal.ConsoleEventArgs args) =>
        {
            if (ZoneSystem.instance == null)
            {
                args.Context.AddString("Erro: ZoneSystem não está disponível");
                return;
            }

            if (!ZNet.instance.IsServer())
            {
                args.Context.AddString("Erro: Este comando só pode ser usado pelo servidor/host");
                return;
            }
            ...
            try
            {
                ...
                List<string> customLocationNames = registeredLocations.Select(l => l.location.name).ToList();

                // Remove instances antigas das locations customizadas
                List<Vector2i> toRemove = new();
                foreach (var kvp in zs.m_locationInstances)
                {
                    if (customLocationNames.Contains(kvp.Value.m_location.m_prefabName))
                    {
                ...
                {
                    zs.m_locationInstances.Remove(key);
                }

                args.Context.AddString($"Removidas {toRemove.Count} instances antigas");

                // FORÇA regeneração completa limpando TUDO
                zs.m_generatedZones.Clear();
                zs.m_locationsByHash.Clear();

                // Re-registra as locations customizadas
                foreach (Location location in registeredLocations)
                {
                    ZoneSystem.ZoneLocation zoneLoc = zs.m_locations.FirstOrDefault(l => l.m_prefabName == location.location.name);
                    if (zoneLoc != null)
                    {
                        zoneLoc.m_quantity = location.Count; // Garante que vai tentar gerar novamente
                    }
                }

                // Chama o método privado que realmente gera as locations
                MethodInfo generateMethod = typeof(ZoneSystem).GetMethod("GenerateLocations",
                    BindingFlags.NonPublic | BindingFlags.Instance);

                if (generateMethod != null)
                {
                    generateMethod.Invoke(zs, new object[] { null });
                    args.Context.AddString("Locations regeneradas com sucesso!");
                }
                else
                {
                    // Fallback: tenta o método público
                    zs.GenerateLocations();
                    args.Context.AddString("Locations regeneradas (método público)!");
                }

                args.Context.AddString("IMPORTANTE: Você precisa explorar NOVAS áreas do mapa!");
                args.Context.AddString("As locations só aparecem em zonas não visitadas anteriormente.");
            }
            catch (Exception e)
            {
                args.Context.AddString($"Erro: {e.Message}");
                Debug.LogError($"Erro completo em regen_locations: {e}");
            }
        });
    }
    ...
    public static class PrefabManager
    {
        private struct BundleId
        {
            ...
            {
                if (obj is BundleId other)
                {
                    return assetBundleFileName == other.assetBundleFileName && folderName == other.folderName;
                }
                return false;
            }
            ...
            {
                return (assetBundleFileName + folderName).GetHashCode();
            }
        }
