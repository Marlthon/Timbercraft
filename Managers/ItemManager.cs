...
public class Item
{
    ...
    // Propriedades para localização de Nome e Descrição
    ...
    public LocalizeKey Name
    {
        get
        {
            ...
            ItemDrop.ItemData data = Prefab.GetComponent<ItemDrop>().m_itemData;
            if (data.m_shared.m_name.StartsWith("$"))
            {
                _name = new LocalizeKey(data.m_shared.m_name);
            }
            else
            {
                string key = "$item_" + Prefab.name.Replace(" ", "_");
                _name = new LocalizeKey(key).English(data.m_shared.m_name);
                data.m_shared.m_name = key;
            }
    ...
    public LocalizeKey Description
    {
        get
        {
            ...
            ItemDrop.ItemData data = Prefab.GetComponent<ItemDrop>().m_itemData;
            if (data.m_shared.m_description.StartsWith("$"))
            {
                _description = new LocalizeKey(data.m_shared.m_description);
            }
            else
            {
                string key = "$item_" + Prefab.name.Replace(" ", "_") + "_description";
                _description = new LocalizeKey(key).English(data.m_shared.m_description);
                data.m_shared.m_description = key;
            }
...
public static class PrefabManager
{
    static PrefabManager()
    {
        ...
        harmony.Patch(AccessTools.DeclaredMethod(typeof(Localization), nameof(Localization.SetupLanguage)), postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(LocalizeKey), nameof(LocalizeKey.AddLocalizedKeys))));
    }
